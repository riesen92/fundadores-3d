...
const props = withDefaults(defineProps<{ selectedFirefighter?: number | null, followSelected?: boolean, hideRoutes?: boolean }>(), { followSelected: false, hideRoutes: false })
...
watch(() => store.rutasBomberos, value => manager?.setFirefighterRoutesVisible(!props.hideRoutes&&value))
watch(() => store.cierres, value => { if (manager) manager.cancha.cierres.visible = value })
watch(() => [firefighters.bomberos,sim.bomberoSeleccionado,sim.actual,props.selectedFirefighter,props.followSelected,props.hideRoutes],refresh,{deep:true})
