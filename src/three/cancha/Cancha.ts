...
import { LLAVES_AGUA, MANGUERAS, PITONES_AGUA, posicionMaterialAgua } from '../../simulation/water'
export class Cancha extends THREE.Group {
  readonly etiquetas = new THREE.Group()
  readonly etiquetasElementos = new THREE.Group()
  readonly cierres = new THREE.Group()
  readonly blancos: Blanco[] = []
  readonly objetos: Record<string, THREE.Group> = {}
  readonly objetosAgua = new Map<string, THREE.Object3D>()
  readonly herramientasIndividuales = new Map<string, THREE.Group>()
  constructor() {
    super(); this.add(this.etiquetas, this.etiquetasElementos, this.cierres)
    ...
    box(this, [0.15, A.puerta.alto, A.puerta.ancho], [0, A.puerta.alto / 2, A.puerta.posicion.z], '#926b4d').name = 'puerta-movil'
    ...
    for (const key of ['gemelo', 'bomba', 'victima', 'camilla', 'herramientas'] as const) { const e = E[key]; const group = new THREE.Group(); this.objetos[key] = group; this.add(group); box(group, e.dimensiones, [e.posicion.x, e.dimensiones[1]! / 2, e.posicion.z], e.color); const texto = label(e.nombre, e.posicion.x, 2.8, e.posicion.z, 6); texto.name = `elemento-${key}`; this.etiquetasElementos.add(texto) }
    ...
    const head = new THREE.Mesh(new THREE.SphereGeometry(0.22), new THREE.MeshStandardMaterial({ color: '#e3aa72' })); head.position.set(E.victima.posicion.x, 0.23, E.victima.posicion.z - 1.05); this.objetos.victima!.add(head)
    for (const side of [-1, 1]) box(this.objetos.camilla!, [0.1, 0.25, 2.3], [E.camilla.posicion.x + side * 0.5, 0.35, E.camilla.posicion.z], '#b6653e')
    for (const dx of [-0.45, 0.45]) { box(this.objetos.herramientas!, [0.07, 0.12, 1.1], [E.herramientas.posicion.x + dx, 0.22, E.herramientas.posicion.z], '#3c4943'); box(this.objetos.herramientas!, [0.4, 0.14, 0.16], [E.herramientas.posicion.x + dx, 0.23, E.herramientas.posicion.z - 0.5], '#d4dad2') }
    for (const key of ['TNT', 'Halligan']) {
      const group = new THREE.Group(); group.visible = false
      box(group, [0.09, 0.12, 1.1], [0, 0.1, 0], '#3c4943')
      box(group, [key === 'TNT' ? 0.5 : 0.3, 0.16, 0.2], [0, 0.12, -0.5], '#d4dad2')
      this.herramientasIndividuales.set(key, group); this.add(group)
      const texto = label(key, 0, 2.8, 0, 3); texto.name = `herramienta-${key}`; texto.visible = false; this.etiquetasElementos.add(texto)
    }
    ...
    for (const id of MANGUERAS) { const roll = new THREE.Mesh(new THREE.TorusGeometry(0.35, 0.12, 8, 24), new THREE.MeshStandardMaterial({ color: '#d92323' })); roll.rotation.x = Math.PI / 2; const p = posicionMaterialAgua(id); roll.position.set(p.x, p.y, p.z); roll.name = `agua-${id}`; this.objetosAgua.set(id, roll); this.add(roll) }
    for (const id of PITONES_AGUA) { const p = posicionMaterialAgua(id), nozzle = box(this, [0.15, 0.15, 0.8], [p.x, p.y, p.z], '#414b48'); nozzle.name = `agua-${id}`; this.objetosAgua.set(id, nozzle) }
    for (const id of LLAVES_AGUA) { const p = posicionMaterialAgua(id), key = box(this, [0.35, 0.09, 0.15], [p.x, p.y, p.z], '#d7ded7'); key.name = `agua-${id}`; this.objetosAgua.set(id, key) }
