...
export class SceneManager {
  ...
  showSimulation(engine:Omit<TaskEngine,never>, selected:number|null){for(const [id,p] of Object.entries(engine.poses))this.firefighters.pose(Number(id),p);this.choreography.renderState(engine,this.cancha,selected)}
  ...
  updateFirefighters(bomberos:Bombero[],selected:number){this.firefighters.update(bomberos,selected)}
